...
# ============================================================
# Lab 10 — Battle System (Polymorphism) Test Runner
# ============================================================
...
BUILD_DIR="$ROOT_DIR/build/battle"
...
SRC_DIR="$ROOT_DIR/assignments/battle"
TEST="$ROOT_DIR/tests/BattleTest.java"
...
compile_all() {
    mkdir -p "$BUILD_DIR"
    echo -e "${DIM}[1/3] Battle source файлуудыг компайл хийж байна...${RESET}"
    javac -d "$BUILD_DIR" "$SRC_DIR"/*.java 2>&1
    if [ $? -ne 0 ]; then
        echo -e "${RED}  ✗ Battle компайл алдаатай!${RESET}"
        exit 1
    fi
    echo -e "${GREEN}  ✓ Battle source компайл амжилттай${RESET}"

    echo -e "${DIM}[2/3] BattleTest.java компайл хийж байна...${RESET}"
...
main() {
    echo -e "${BOLD}${MAGENTA}"
    echo "  ╔═══════════════════════════════════════════════╗"
    echo "  ║  🐉 Lab 10 — Battle System Polymorphism      ║"
    echo "  ╚═══════════════════════════════════════════════╝"
    echo -e "${RESET}"
    ...
    for f in "$SRC_DIR/Warrior.java" "$SRC_DIR/Mage.java" "$SRC_DIR/Rogue.java" "$SRC_DIR/Battle.java"; do
        if [ -f "$f" ]; then
